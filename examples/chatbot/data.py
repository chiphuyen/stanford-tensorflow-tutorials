...
def build_vocab(filename, vocab_size, normalize_digits=True):
...
def process_data():
    # build_vocab('train.enc', config.ENC_VOCAB)
    # build_vocab('train.dec', config.DEC_VOCAB)
    token2id('train', 'enc')
    token2id('train', 'dec')
    token2id('test', 'enc')
    token2id('test', 'dec')
...
def get_batch(data_bucket, bucket_id, batch_size=1):
    ...
    # has to subtract 1 because of the way we divide samples into buckets (make it < size)
